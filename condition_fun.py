...
def check_datetime_cols(dat):
    datetime_cols = dat.apply(pd.to_numeric, errors='ignore').select_dtypes(object).apply(pd.to_datetime,
                                                                                          errors='ignore').select_dtypes(
        'datetime64').columns.tolist()
...
def rep_blank_na(dat):  # cant replace blank string in categorical value with nan
    ...
    blank_cols = [i for i in list(dat) if
                  dat[i].astype(str).str.findall(r'^\s*$').apply(lambda x: 0 if len(x) == 0 else 1).sum() > 0]
    if len(blank_cols) > 0:
